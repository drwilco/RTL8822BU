...
#define DBG_MGMT_XMIT_ENC_DUMP 0
	...
	if (bmcst) {
		if (subtype == WIFI_ACTION && CATEGORY_IS_GROUP_PRIVACY(category)) {
			...
			if (psta == NULL) {
				RTW_ERR(FUNC_ADPT_FMT" broadcast sta == NULL\n"
					, FUNC_ADPT_ARG(padapter));
				goto xmitframe_coalesce_fail;
			}
			if (padapter->securitypriv.binstallGrpkey != _TRUE) {
				RTW_ERR(FUNC_ADPT_FMT" GTK is not installed\n"
					, FUNC_ADPT_ARG(padapter));
				goto xmitframe_coalesce_fail;
			}
			...
			if (subtype == WIFI_DEAUTH)
				RTW_DBG(FUNC_ADPT_FMT" broadcast deauth\n", FUNC_ADPT_ARG(padapter));
			else if (subtype == WIFI_DISASSOC)
				RTW_DBG(FUNC_ADPT_FMT" broadcast disassoc\n", FUNC_ADPT_ARG(padapter));
			else if (subtype == WIFI_ACTION) {
				RTW_DBG(FUNC_ADPT_FMT" broadcast action(%u)\n"
					, FUNC_ADPT_ARG(padapter), category);
			}
			#endif
	...
	*/
	...
	if (bmcst) {
		...
		if (rtw_tim_map_anyone_be_set(padapter, pstapriv->sta_dz_bitmap)) { /* if anyone sta is in ps mode */
			...
			RTW_DBG("enqueue, sq_len=%d\n", psta->sleepq_len);
			RTW_DBG_DUMP("enqueue, tim=", pstapriv->tim_bitmap, pstapriv->aid_bmp_len);
			if (update_tim == _TRUE) {
