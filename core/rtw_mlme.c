...
static struct sta_info *rtw_joinbss_update_stainfo(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	if (psta) { /* update ptarget_sta */
		...
		for (i = 0; i < 16 ; i++) {
			...
			#ifdef DBG_RX_SEQ
			RTW_DBG("DBG_RX_SEQ "FUNC_ADPT_FMT" tid:%u SN_CLEAR indicate_seq:%u\n"
...
void rtw_sta_mstatus_disc_rpt(_adapter *adapter, u8 mac_id)
{
	...
	if (mac_id >= 0 && mac_id < macid_ctl->num) {
		...
		RTW_DBG(FUNC_ADPT_FMT" - mac_id=%d%s\n", FUNC_ADPT_ARG(adapter)
...
u8 rtw_is_adapter_up(_adapter *padapter)
{
	...
	if (RTW_CANNOT_RUN(padapter)) {
		RTW_DBG(FUNC_ADPT_FMT " FALSE -bDriverStopped(%s) bSurpriseRemoved(%s)\n"
			, FUNC_ADPT_ARG(padapter)
	...
	if (!rtw_is_hw_init_completed(padapter)) {
		RTW_DBG(FUNC_ADPT_FMT " FALSE -(hw_init_completed == _FALSE)\n", FUNC_ADPT_ARG(padapter));
		return _FALSE;
	}

	if (padapter->bup == _FALSE) {
		RTW_DBG(FUNC_ADPT_FMT " FALSE -(bup == _FALSE)\n", FUNC_ADPT_ARG(padapter));
		return _FALSE;
	}
