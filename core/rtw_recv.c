...
void process_pwrbit_data(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *psta)
{
	...
	if (pwrbit) {
		...
		if (psta->state & WIFI_SLEEP_STATE) {
			...
			RTW_DBG_DUMP("to wakeup, sta_dz_bitmap=", pstapriv->sta_dz_bitmap, pstapriv->aid_bmp_len);
		}
	}
#endif
}
	...
	struct sta_info **psta)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE)
		...
		if (_rtw_memcmp(myhwaddr, pattrib->src, ETH_ALEN)) {
			...
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" SA="MAC_FMT", myhwaddr="MAC_FMT"\n"
		...
		if ((!_rtw_memcmp(myhwaddr, pattrib->dst, ETH_ALEN)) && (!bmcast)) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" DA="MAC_FMT"\n"
		...
		if (_rtw_memcmp(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
		    ...
		    (!_rtw_memcmp(pattrib->bssid, mybssid, ETH_ALEN))) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" BSSID="MAC_FMT", mybssid="MAC_FMT"\n"
		...
		*psta = rtw_get_stainfo(pstapriv, pattrib->ta);
		if (*psta == NULL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" can't get psta under STATION_MODE ; drop pkt\n"
		...
		*psta = rtw_get_stainfo(pstapriv, pattrib->bssid); /* get sta_info */
		if (*psta == NULL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" can't get psta under WIFI_MP_STATE ; drop pkt\n"
				, FUNC_ADPT_ARG(adapter));
			#endif
			ret = _FAIL;
			goto exit;
		...
		ret = _FAIL;
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" fw_state:0x%x\n"
	...
	if (get_frame_sub_type(pframe) == WIFI_PSPOLL) {
		...
		if ((psta->state & WIFI_SLEEP_STATE) && (rtw_tim_map_is_set(padapter, pstapriv->sta_dz_bitmap, psta->cmn.aid))) {
			...
			if ((rtw_end_of_queue_search(xmitframe_phead, xmitframe_plist)) == _FALSE) {
				...
				RTW_DBG("handling ps-poll, q_len=%d\n", psta->sleepq_len);
				RTW_DBG_DUMP("handling, tim=", pstapriv->tim_bitmap, pstapriv->aid_bmp_len);
				...
				if (psta->sleepq_len == 0) {
					...
					RTW_DBG("after handling ps-poll\n");
					RTW_DBG_DUMP("after handling, tim=", pstapriv->tim_bitmap, pstapriv->aid_bmp_len);
...
pre_validate_status_chk:
	if (ret == _FAIL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" case:%d, res:%d, ra="MAC_FMT", ta="MAC_FMT"\n"
	...
	if (psta == NULL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" psta == NULL, ra="MAC_FMT", ta="MAC_FMT"\n"
	...
	if (!IS_MCAST(pattrib->ra)) {
		...
		if (recv_decache(precv_frame, psta->sta_recvpriv.rxcache.tid_rxseq) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_decache uc return _FAIL for sta="MAC_FMT"\n"
		...
		if (recv_ucast_pn_decache(precv_frame) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_ucast_pn_decache return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
		...
		if (recv_decache(precv_frame, psta->sta_recvpriv.bmc_tid_rxseq) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_decache bmc return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
		...
		if (recv_bcast_pn_decache(precv_frame) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_bcast_pn_decache return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
...
sint validate_recv_frame(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	default:
		...
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" fail! type=0x%x\n"
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	while (a_len > ETH_HLEN) {
		...
		if (sub_pkt == NULL) {
			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_DBG("DBG_RX_DROP_FRAME %s rtw_os_alloc_msdu_pkt fail\n", __func__);
				#endif
			}
			if (act & RTW_RX_MSDU_ACT_FORWARD) {
				#ifdef DBG_TX_DROP_FRAME
				RTW_DBG("DBG_TX_DROP_FRAME %s rtw_os_alloc_msdu_pkt fail\n", __func__);
				#endif
...
static int check_indicate_seq(struct recv_reorder_ctrl *preorder_ctrl, u16 seq_num)
{
	...
	if (preorder_ctrl->indicate_seq == 0xFFFF) {
		...
		#ifdef DBG_RX_SEQ
		RTW_DBG("DBG_RX_SEQ "FUNC_ADPT_FMT" tid:%u SN_INIT indicate_seq:%d, seq_num:%d\n"
	...
	if (SN_LESS(seq_num, preorder_ctrl->indicate_seq)) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG(FUNC_ADPT_FMT" tid:%u indicate_seq:%d > seq_num:%d\n"
			, FUNC_ADPT_ARG(padapter), preorder_ctrl->tid, preorder_ctrl->indicate_seq, seq_num);
		#endif
		return _FALSE;
	}
	...
	*/
	if (SN_EQUAL(seq_num, preorder_ctrl->indicate_seq)) {
		...
		#ifdef DBG_RX_SEQ
		RTW_DBG("DBG_RX_SEQ "FUNC_ADPT_FMT" tid:%u SN_LESS(wend, seq_num) indicate_seq:%d, seq_num:%d\n"
			, FUNC_ADPT_ARG(padapter), preorder_ctrl->tid, preorder_ctrl->indicate_seq, seq_num);
		#endif
	}
...
static int recv_indicatepkts_in_order(_adapter *padapter, struct recv_reorder_ctrl *preorder_ctrl, int bforced)
{
	...
	if (bforced == _TRUE) {
		...
		#ifdef DBG_RX_SEQ
		RTW_DBG("DBG_RX_SEQ "FUNC_ADPT_FMT" tid:%u FORCE indicate_seq:%d, seq_num:%d\n"
	...
	while (!rtw_is_list_empty(phead)) {
		...
		if (!SN_LESS(preorder_ctrl->indicate_seq, pattrib->seq_num)) {
			...
			if (SN_EQUAL(preorder_ctrl->indicate_seq, pattrib->seq_num)) {
				preorder_ctrl->indicate_seq = (preorder_ctrl->indicate_seq + 1) & 0xFFF;
				#ifdef DBG_RX_SEQ
				RTW_DBG("DBG_RX_SEQ "FUNC_ADPT_FMT" tid:%u SN_EQUAL indicate_seq:%d, seq_num:%d\n"
					, FUNC_ADPT_ARG(padapter), preorder_ctrl->tid, preorder_ctrl->indicate_seq, pattrib->seq_num);
				#endif
			}
			...
			if (index == 1) {
				...
				if (bforced != _TRUE) {
					RTW_DBG("_cancel_timer_ex(&preorder_ctrl->reordering_ctrl_timer);\n");
...
static int recv_indicatepkt_reorder(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (!check_indicate_seq(preorder_ctrl, pattrib->seq_num)) {
		...
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" check_indicate_seq fail\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
#if 0
	...
	if (!enqueue_reorder_recvframe(preorder_ctrl, prframe)) {
		...
		/* return _FAIL; */
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" enqueue_reorder_recvframe fail\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
		goto _err_exit;
	}
...
static void recv_set_iseq_before_mpdu_process(union recv_frame *rframe, u16 seq_num, const char *caller)
{
	...
	if (reorder_ctrl) {
		...
		#ifdef DBG_RX_SEQ
		RTW_DBG("DBG_RX_SEQ %s("ADPT_FMT")-B tid:%u indicate_seq:%d, seq_num:%d\n"
	...
	if ((check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE)) { /* &&(padapter->mppriv.check_mp_pkt == 0)) */
		...
		if (pmppriv->rx_bindicatePkt == _FALSE) {
			...
			if (type == WIFI_DATA_TYPE) {
				...
				if (ret != _SUCCESS) {
					#ifdef DBG_RX_DROP_FRAME
					RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" wlanhdr_to_ethhdr: drop pkt\n"
						, FUNC_ADPT_ARG(padapter));
					#endif
					rtw_free_recvframe(rframe, pfree_recv_queue);/* free this recv_frame */
					ret = _FAIL;
				...
				if (!RTW_CANNOT_RUN(padapter)) {
					...
					if (ret != _SUCCESS) {
						#ifdef DBG_RX_DROP_FRAME
						RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" rtw_recv_indicatepkt fail!\n"
							, FUNC_ADPT_ARG(padapter));
						#endif
						rtw_free_recvframe(rframe, pfree_recv_queue);/* free this recv_frame */
						ret = _FAIL;
					...
					#ifdef DBG_RX_DROP_FRAME
					RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" bDriverStopped(%s) OR bSurpriseRemoved(%s)\n"
						, FUNC_ADPT_ARG(padapter)
...
int recv_func_posthandle(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (prframe == NULL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" decryptor: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
		ret = _FAIL;
	...
	if (prframe == NULL)	{
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recvframe_chk_defrag: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
	...
	if (prframe == NULL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_DBG("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" portctrl: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
		ret = _FAIL;
