...
void set_channel_bwmode(_adapter *padapter, unsigned char channel, unsigned char channel_offset, unsigned short bwmode)
{
	...
	if (MCC_EN(padapter)) {
		...
		if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC))
			RTW_WARN("Warning: Do not set channel setting reg MCC mode\n");
	}
#endif
...
s16 rtw_get_camid(_adapter *adapter, u8 *addr, s16 kid, u8 gk)
{
	...
	if (i == cam_ctl->num) {
		RTW_WARN(FUNC_ADPT_FMT" %s key with "MAC_FMT" id:%u no room\n"
...
assigned:
	psta->cmn.mac_id = i;
	RTW_DBG(FUNC_ADPT_FMT" if%u, mac_addr:"MAC_FMT" macid:%u\n"
